...
from lora_utils import apply_lora_to_model, freeze_non_lora_params, print_trainable_params
...
class Model(pl.LightningModule):
    def __init__(self, baseline, dense_setting, attn_save_dir, n_layers, last_layer_n_heads, dim_hidden, dim_qk, dim_v,
                 ...
                 maximum_node=50, save_display=False, save_display_interval=100, lora=False, lora_rank=4, lora_alpha=8.0, lora_dropout=0.0, lora_target="attn",):
        super().__init__()
        ...
        if self.lora:
            adapted_layers = apply_lora_to_model(
                self.encoder,
                rank=self.lora_rank,
                alpha=self.lora_alpha,
                dropout=self.lora_dropout,
                target=self.lora_target,
            )
            freeze_non_lora_params(self)
            print("Applied LoRA to these layers:")
            for layer_name in adapted_layers:
                print("  -", layer_name)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--lora_target', type=str, default="attn", choices=["attn", "all"])
